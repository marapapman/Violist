...
public class WAMConfigParser extends DefaultHandler {
    ...
    public void startElement(String namespaceURI, String lName, String qName, Attributes attrs) throws SAXException {   
        if (qName.equalsIgnoreCase("app")) {
        	...
        	debug("\tJar: " + jar);
        } else if (qName.equalsIgnoreCase("mainmethod")) {
        	String subsig=attrs.getValue("subsig");
        	String classname=attrs.getValue("classname");
        	ConfMainMethod mainm=new  ConfMainMethod();
        	mainm.classname=classname;
        	mainm.subsig=subsig;
        	config.addMainMethod(mainm);
        }
    }
