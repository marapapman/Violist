...
public class InterRe {
	...
	public InterRe(List<NodeInterface> nodes,Map<String,Boolean> regionDef,List<String> lineDef,String folderName,Map<NodeInterface,List<String>> targetVarNodeAndName, Map<String, Set<NodeInterface>> paraMap,Map<String,Set<String>> field,JavaApp App)
	{
	...
	//		System.out.println(n.getOffset().toString()+" "+actualNode.toString());
		...
		if(actualNode!=null&&lineDef.contains(n.getOffset().toString()))
		{
			...
			if(actualNode.toString().contains("@parameter"))
				lineUseMap.put(n.getOffset().toString(),new ExternalPara(sootUse.get(0).getValue().toString()));
			...
			//case : valueOf or toString
					...
					||actualNode.toString().contains("<java.lang.StringBuilder: java.lang.String toString()")||actualNode.toString().contains("<java.lang.StringBuffer: java.lang.String toString()>()"))
			{
			...
			{				
				...
				for(ValueBox vb:sootUse)
				{
					...
					if(!use.contains("virtualinvoke"))
					{
						...
						else if((use.contains("b")||use.contains("i"))||use.contains("c")||use.contains("z")&&!use.contains("\""))
						{
					...
					else
					{
						...
						else if(use.contains("<java.lang.StringBuilder: java.lang.StringBuilder append")||use.contains("<java.lang.StringBuffer: java.lang.StringBuffer append"))							
			...
			else if(actualNode.toString().contains("staticinvoke")||actualNode.toString().contains("specialinvoke")||actualNode.toString().contains("virtualinvoke")||actualNode.toString().contains("interfaceinvoke"))
			{
				...
				//	fileName = Encoder.query("<testcases.standalone.InterfaceMethod$InnerClass: java.lang.String foo()>") + ".ser";
				}
			...
			if(actualNode.getUseBoxes().get(0).getValue().getType().toString().equals("java.lang.String"))		
			{
				...
				else
				{
					if(regionDef.get(returnNodeName)==null)
						returnVarNodeAndName.put(n, new ExternalPara("Unknown"));
					else
					{
		...
		else if(actualNode!=null&&(actualNode.toString().contains("staticinvoke")||actualNode.toString().contains("virtualinvoke")||actualNode.toString().contains("specialinvoke")||actualNode.toString().contains("specialinvoke"))&&!actualNode.toString().contains("<init>")&&!actualNode.toString().contains("goto"))
		{
			...
			int index1 = method.indexOf("<");
			int index2 = method.indexOf(">");
			String signature = method.substring(index1,index2+1);
			if(!paraMap.containsKey(signature))
			{
				Set<NodeInterface> tempSet = new HashSet<>();
				tempSet.add(n);
