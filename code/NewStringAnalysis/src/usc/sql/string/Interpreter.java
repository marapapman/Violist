...
public class Interpreter {
	...
	private Set<String> interpret(Variable ir,int itr)
	{
		if(ir instanceof ConstantInt)
		{
			...
			//System.out.println("here"+ir.getValue());
			Set<String> s = new HashSet<>();
			//s.add(ir.getValue());
			
			try
			{
				String temp = ""+(char)Integer.parseInt(ir.getValue());
				if(temp.length()>0)
					s.add( temp);
		...
		else if(ir instanceof ExternalPara)
		{
			...
			{
				/*
				if(fieldMap.get(((ExternalPara) ir).getName())!=null)
				{
					for(String temp:fieldMap.get(((ExternalPara) ir).getName()))
					{
						if(temp.matches("<.*>"))
							s.addAll(fieldMap.get(temp));
						else
							s.add(temp);
					}
					//s.addAll(fieldMap.get(((ExternalPara) ir).getName()));
				}
				else if(((ExternalPara) ir).getName().contains("<"))
				{
					s.add("null");
				}
				*/
